...
# NEW: Strictly limit syncing and purging to these exact folders
TARGET_DIRS = ["config", "resourcepacks", "mods"]
# ---------------------
...
def get_policy_for_file(filename, relative_path, custom_policies):
    ...
    # PRIORITY 3: Default to Strict
    # Since we are pre-filtering folders, anything inside config/mods/resourcepacks 
    # defaults to strict unless overridden by policies.json.
...
def generate_manifest():
    ...
    manifest = {
        ...
        "files": [],
        "managed_directories": TARGET_DIRS, # This drives the strict purge in C#
    }
    ...
    for root, _, files in os.walk(MODPACK_FOLDER):
        for file in files:
            ...
            # Ensure safe path string handling regardless of OS
            forward_slash_path = "/".join(relative_path.split(os.sep))

            # ENFORCEMENT: Skip any file that does not live inside our target directories
            is_targeted = any(forward_slash_path.startswith(d + "/") for d in TARGET_DIRS)
            if not is_targeted:
                continue
    ...
    print(f"Set hai boss! {len(manifest['files'])} targeted files ka '{OUTPUT_FILE}' ready hai.")
